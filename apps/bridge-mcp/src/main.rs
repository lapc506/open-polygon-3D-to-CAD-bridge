use anyhow::Result;
use cfd_core::{CfdStudy, FluidMaterial, MisterInjection, Point3, Vector3, DropletDistribution, DistributionSource};
...
fn demo_study() -> CfdStudy {
    CfdStudy {
        ...
        injections: vec![MisterInjection {
            ...
            distribution: DropletDistribution { d10_m: Some(10e-6), d50_m: Some(30e-6), d90_m: Some(60e-6), source: DistributionSource::Assumed },
            duty_cycle: Some(1.0),
        }],
...
fn handle(req: Request) -> Result<Value> {
    let result = match req.method.as_str() {
        "tools/list" => tools(),
        "cfd.create_study" => serde_json::to_value(demo_study())?,
        "cfd.prepare" => {
            let study: CfdStudy = if req.params.is_object() && req.params.get("study").is_some() {
                serde_json::from_value(req.params["study"].clone())?
            } else { demo_study() };
            let backend = OpenFoamBackend::new("runs/openfoam");
            serde_json::to_value(backend.prepare(&study).map_err(|e| anyhow::anyhow!(e))?)?
        }
        "model.validate" => {
            let study: CfdStudy = serde_json::from_value(req.params.get("study").cloned().unwrap_or_else(|| serde_json::to_value(demo_study()).unwrap()))?;
            json!({"valid": study.validate().is_ok()})
        }
        "model.inspect" => json!({"status":"scaffold","canonical_model":"CIM","simulation_backend":"OpenFOAM","physical_actuation":false}),
...
fn main() -> Result<()> {
    ...
    for line in stdin.lock().lines() {
        ...
        let response = match handle(req) {
            Ok(result) => Response { id: Value::Null, result: Some(result), error: None },
            Err(error) => Response { id: Value::Null, result: None, error: Some(json!({"message": error.to_string()})) },
        };
